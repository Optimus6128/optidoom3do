...
Word columnWidth = 1;                       // column width in pixels (1 for fullRes, 2 for halfRes)
...
static void DrawSeg(viswall_t *segl)
{
	...
	if (ActionBits & (AC_TOPTEXTURE|AC_BOTTOMTEXTURE)) {
		...
		_scalestep = segl->ScaleStep << (columnWidth - 1);
...
if (ActionBits & AC_ADDSKY) skyOn = true;

		if ((ActionBits & AC_ADDSKY) && (skyType==SKY_DEFAULT)) {
			int bottom;
...
// Shall I add the floor?
		...
		if (ActionBits & AC_ADDFLOOR) {
			...
			if (top <= bottom) {		// Valid span?
				...
				FloorPlane->open[x] = (top<<8)+bottom;	// Set the new vertical span
				if (x<segl->RightX) FloorPlane->open[x+1] = FloorPlane->open[x];
			}
		}
...
// Handle ceilings
		...
		if (ActionBits & AC_ADDCEILING) {
			...
			if (top <= bottom) {
				...
				CeilingPlane->open[x] = (top<<8)+bottom;		// Set the vertical span
				if (x<segl->RightX) CeilingPlane->open[x+1] = CeilingPlane->open[x];
			}
		}
...
// Sprite clip sils
		...
		if (ActionBits & (AC_BOTTOMSIL|AC_NEWFLOOR)) {
			...
			if (ActionBits & AC_BOTTOMSIL) {
				segl->BottomSil[x] = low;
				if (x<segl->RightX) segl->BottomSil[x+1] = low;
			}
			if (ActionBits & AC_NEWFLOOR) {
				clipboundbottom[x] = low;
				if (x<segl->RightX) clipboundbottom[x+1] = low;
			}
		}
		...
		if (ActionBits & (AC_TOPSIL|AC_NEWCEILING)) {
			...
			if (ActionBits & AC_TOPSIL) {
				segl->TopSil[x] = high+1;
				if (x<segl->RightX) segl->TopSil[x+1] = high+1;
			}
			if (ActionBits & AC_NEWCEILING) {
				clipboundtop[x] = high;
				if (x<segl->RightX) clipboundtop[x+1] = high;
			}
		}
...
{
	{
        ...
        if (cheatNoclipEnabled) DrawARect(0,0, ScreenWidth, ScreenHeight, 0);   // To avoid HOM when noclipping outside

        if (skyOn && (skyType!=SKY_DEFAULT)) drawNewSky(skyType);
        skyOn = false;
        ...
        if (wallQuality == WALL_QUALITY_LO_FAST) {
            do {
                SegLoopLo(WallSegPtr);			// Create the viswall records and draw the sky only
